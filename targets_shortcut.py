...
def get_targets(FLAGS, key, train_state, images, labels, force_t=-1, force_dt=-1):
    ...
    if not FLAGS.model['bootstrap_cfg']:
        v_b1 = call_model_fn(x_t, t, dt_base_bootstrap,
                             bst_labels, train=False)
        t2 = t + dt_bootstrap
        x_t2 = x_t + dt_bootstrap[:, None, None, None] * v_b1
        x_t2 = jnp.clip(x_t2, -4, 4)
        v_b2 = call_model_fn(x_t2, t2, dt_base_bootstrap,
                             bst_labels, train=False)
        ...
        labels_extra = jnp.concatenate([bst_labels, jnp.ones(
            num_dt_cfg, dtype=jnp.int32) * FLAGS.model['num_classes']], axis=0)
        v_b1_raw = call_model_fn(
        ...
        v_b2_raw = call_model_fn(x_t2_extra, t2_extra,
                                 dt_base_extra, labels_extra, train=False)
