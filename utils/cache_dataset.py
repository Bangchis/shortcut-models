...
class CacheDatasetIterator:
    ...
    4. If posterior mode: x1 = μ + vae_epsilon_scale·σ·ε (stochastic)
    5. Yield (x1, k_vec)

    This implements the paper's mixed batch sampling approach:
    - Sample cluster k ~ π
    - Sample data index uniformly from D_k
    - x1 = cached latent (with stochastic augmentation if posterior mode)
    ...
    def __init__(
        ...
        """
        Args:
            latent_cache: Memory-mapped latents (N, H, W, C) OR
            ...
            vae_epsilon_scale: Stochasticity parameter τ (0.0=deterministic, 1.0=full posterior)
        """
        # Handle both single memmap and tuple input
        if isinstance(latent_cache, tuple):
            self.mu_cache, self.logvar_cache = latent_cache
        ...
        self.vae_epsilon_scale = vae_epsilon_scale
    ...
    def __next__(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Gather latents from cache with stochastic augmentation if posterior mode
        if self.is_posterior:
            # Posterior mode: x1 = μ + τ·exp(0.5·logvar)·ε
            mu = self.mu_cache[indices].astype(np.float32)  # (B, H, W, C)
            logvar = self.logvar_cache[indices].astype(np.float32)  # (B, H, W, C)

            if self.vae_epsilon_scale == 0.0:
                # Deterministic: use mean only
                x1 = mu
            else:
                # Stochastic: sample from posterior
                epsilon = self.rng.randn(*mu.shape).astype(np.float32)
                sigma = np.exp(0.5 * logvar)
                x1 = mu + self.vae_epsilon_scale * sigma * epsilon
        else:
            # Legacy mode: use fixed cached latents
...
class CacheDatasetIteratorWithLabels(CacheDatasetIterator):
    ...
    def __init__(
        ...
        Args:
            ...
            vae_epsilon_scale: Stochasticity parameter τ
        """
        super().__init__(latent_cache, cluster_cache, batch_size, rng_seed, vae_epsilon_scale)
    ...
    def __next__(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Gather latents with stochastic augmentation if posterior mode
        if self.is_posterior:
            # Posterior mode: x1 = μ + τ·exp(0.5·logvar)·ε
            mu = self.mu_cache[indices].astype(np.float32)
            logvar = self.logvar_cache[indices].astype(np.float32)
