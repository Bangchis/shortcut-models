...
def main(_):
    ...
    def update(train_state, train_state_teacher, images, labels, force_t=-1, force_dt=-1):
        ...
        def loss_fn(grad_params):
            ...
            info = {
                'loss': loss,
                'v_magnitude_prime_RMS': jnp.sqrt(jnp.mean(jnp.square(v_prime))),
                **{'activationsRMS/' + k: jnp.sqrt(jnp.mean(jnp.square(v))) for k, v in activations.items()},
                # --- NEW -------------------------------------
                'v_magnitude_prime_L2_norm': jnp.sqrt(jnp.sum(jnp.square(v_prime))),
