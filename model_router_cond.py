...
class RouterCondEmbedder(nn.Module):
    """Embed sparse router posterior into conditioning vector."""
    num_modes: int
    hidden_size: int
    tc: TrainConfig

    @nn.compact
    def __call__(self, router_cond):
        # router_cond: [B, num_modes] sparse probability vector
        x = router_cond.astype(self.tc.dtype)
        x = nn.Dense(self.hidden_size, kernel_init=nn.initializers.normal(0.02),
                     bias_init=self.tc.kern_init('bias', zero=True), dtype=self.tc.dtype)(x)
        x = nn.silu(x)
        x = nn.Dense(self.hidden_size, kernel_init=nn.initializers.normal(0.02),
                     bias_init=self.tc.kern_init('bias', zero=True), dtype=self.tc.dtype)(x)
        return x
...
class DiT(nn.Module):
    ...
    Extended with optional router conditioning for projected_diag_gmm mode.
    """
    patch_size: int
    hidden_size: int
    depth: int
    ...
    @nn.compact
    def __call__(self, x, t, dt, y, router_cond=None, train=False, return_activations=False):
        ...
        # Router conditioning (only when router_cond is provided and num_modes > 0).
        if router_cond is not None and self.num_modes > 0:
            ce = RouterCondEmbedder(self.num_modes, self.hidden_size, tc=tc)(router_cond)
            c = c + ce
